...
const productCreateController = {
  async createProduct(req, res) {
    try {
      ...
      const { productname, price, description, detail, is_newArrivals, is_topSelling, cover_image, first_image, second_image, third_image } = req.body;
      ...
      if (![1, 2, 3, 4].includes(category)) {
        return res.status(400).json({ error: "A categoria deve ser um número entre 1 e 4." });
      }
